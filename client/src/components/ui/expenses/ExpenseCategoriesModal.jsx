...
export default function ExpenseCategoriesModal({ isOpen, onClose }) {
  ...
  const resetToList = () => { setView("list"); setEditing(null); setDeleting(null); setForm(emptyForm); setAlert(null); };
  ...
  const handleDelete = async () => {
    try {
      ...
      setAlert("No se pudo eliminar. Puede tener gastos asociados.");
      setView("list");
      setDeleting(null);
    }
  };
  ...
  const title = view === "form" ? (editing ? "Editar Categoría" : "Nueva Categoría") : view === "delete" ? "Eliminar Categoría" : "Categorías de Gastos";

  return (
    <Modal isOpen={isOpen} onClose={handleClose} title={title}>
      {view === "list" && (
        <div>
          <button onClick={() => { setForm(emptyForm); setEditing(null); setAlert(null); setView("form"); }} className="w-full flex items-center justify-center gap-2 bg-primary text-white px-4 py-2.5 rounded-lg text-sm font-medium hover:bg-primary-dark transition-colors mb-4">
            <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 4v16m8-8H4" /></svg>
            Agregar categoría
          </button>
          {isLoading ? (
            <div className="flex justify-center py-6"><svg className="animate-spin h-6 w-6 text-primary" viewBox="0 0 24 24"><circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4" fill="none" /><path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4z" /></svg></div>
          ) : !categories?.length ? (
      ...
      {view === "form" && (
        <div>
          {alert && (<div className="flex items-center gap-2 bg-red-50 border border-red-200 text-red-700 text-sm px-4 py-3 rounded-lg mb-4"><svg className="w-5 h-5 shrink-0" fill="currentColor" viewBox="0 0 20 20"><path fillRule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zM8.707 7.293a1 1 0 00-1.414 1.414L8.586 10l-1.293 1.293a1 1 0 101.414 1.414L10 11.414l1.293 1.293a1 1 0 001.414-1.414L11.414 10l1.293-1.293a1 1 0 00-1.414-1.414L10 8.586 8.707 7.293z" clipRule="evenodd" /></svg><span>{alert}</span></div>)}
          <form onSubmit={handleSubmit} className="space-y-4">
            <div><label className="block text-sm font-medium text-gray-700 mb-1">Nombre</label><input type="text" value={form.name} onChange={(e) => { setForm({ ...form, name: e.target.value }); setAlert(null); }} required className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-primary focus:border-transparent" /></div>
            <div><label className="block text-sm font-medium text-gray-700 mb-1">Descripción</label><textarea value={form.description} onChange={(e) => setForm({ ...form, description: e.target.value })} rows={2} className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-primary focus:border-transparent resize-none" /></div>
