...
import RawMaterialCategoriesModal from "../../ui/raw-materials/RawMaterialCategoriesModal";
...
export default function RawMaterialsTable() {
  ...
  const { data: categories } = useGetRawMaterialCategoriesQuery();
  const [createMaterial, { isLoading: isCreating }] = useCreateRawMaterialMutation();
  const [updateMaterial, { isLoading: isUpdating }] = useUpdateRawMaterialMutation();
  ...
  const handleSubmit = async (e) => {
    ...
    if (!form.category) { setAlert("Selecciona una categoría"); return; }
    if (!form.unit.trim()) { setAlert("La unidad es requerida"); return; }
    if (!form.price || Number(form.price) <= 0) { setAlert("El precio debe ser mayor a 0"); return; }
    ...
    try {
      if (editing) {
        await updateMaterial({ id: editing.id, ...payload }).unwrap();
      } else {
      ...
      if (typeof msg === "object") {
        ...
        setAlert("Error al guardar insumo");
      }
    }
  };
  ...
  const handleDelete = async () => {
    try {
      await deleteMaterial(deleting.id).unwrap();
      setDeleteModalOpen(false);
      setDeleting(null);
    } catch {
      setAlert("Error al eliminar insumo");
    }
  };
    ...
    <div>
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-gray-500 text-sm mt-1">Gestión de insumos</p>
        </div>
        <div className="flex items-center gap-2">
          <button
            onClick={() => navigate("/inventory/raw-materials/movements")}
          ...
          <button
            ...
            className="flex items-center gap-2 border border-primary text-primary px-4 py-2.5 rounded-lg text-sm font-medium hover:bg-primary-light transition-colors"
          >
            <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
      ...
      <DataTable columns={columns} data={materials} isLoading={isLoading} isError={isError} onEdit={openEdit} onDelete={(row) => { setDeleting(row); setDeleteModalOpen(true); }} />

      <CategoriesAndFormModals
        modalOpen={modalOpen} setModalOpen={setModalOpen}
        editing={editing} form={form} handleChange={handleChange}
        handleSubmit={handleSubmit} isSaving={isSaving} alert={alert}
        categories={categories}
      />

      <RawMaterialCategoriesModal isOpen={categoryModalOpen} onClose={() => setCategoryModalOpen(false)} />

      <Modal isOpen={deleteModalOpen} onClose={() => setDeleteModalOpen(false)} title="Eliminar Insumo">
        <p className="text-gray-600 mb-6">
          ¿Estás seguro de que deseas eliminar <span className="font-semibold text-gray-800">{deleting?.name}</span>? Esta acción no se puede deshacer.
        </p>
...
function CategoriesAndFormModals({ modalOpen, setModalOpen, editing, form, handleChange, handleSubmit, isSaving, alert, categories }) {
  return (
    <Modal isOpen={modalOpen} onClose={() => setModalOpen(false)} title={editing ? "Editar Insumo" : "Nuevo Insumo"}>
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <div>
          ...
          <select name="category" value={form.category} onChange={handleChange} required className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-primary focus:border-transparent bg-white">
        ...
        <div className="grid grid-cols-3 gap-3">
          <div>
            ...
            <input type="number" name="quantity" value={form.quantity} onChange={handleChange} min="0" className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-primary focus:border-transparent" />
          </div>
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">Unidad</label>
            <input type="text" name="unit" value={form.unit} onChange={handleChange} required placeholder="kg, lt, unidad" className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-primary focus:border-transparent" />
          </div>
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">Precio</label>
            <input type="number" name="price" value={form.price} onChange={handleChange} step="0.01" min="0" required className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-primary focus:border-transparent" />
          </div>
        </div>
