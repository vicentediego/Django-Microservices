...
export default function ProductMovementsTable() {
  const { data: movements, isLoading, isError } = useGetProductMovementsQuery();
  const { data: products } = useGetProductsQuery();
  const [createMovement, { isLoading: isCreating }] = useCreateProductMovementMutation();
  const [updateMovement, { isLoading: isUpdating }] = useUpdateProductMovementMutation();
  const [deleteMovement] = useDeleteProductMovementMutation();
    ...
    <div>
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <h1 className="text-2xl font-bold text-gray-800">Movimientos de Producto</h1>
          <p className="text-gray-500 text-sm mt-1">Entradas y salidas de stock</p>
        </div>
        <button
          onClick={() => {
            setEditing(null);
