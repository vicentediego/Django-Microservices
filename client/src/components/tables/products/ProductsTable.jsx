...
import CategoriesModal from "../../ui/products/CategoriesModal";
...
export default function ProductsTable() {
  ...
  const columns = [
    ...
    },
    {
      ...
      label: "Stock",
      render: (row) => (
        <span className={row.stock <= row.min_stock ? "text-red-500 font-medium" : ""}>
          {row.stock}
        </span>
      ),
    },
  ...
  const handleSubmit = async (e) => {
    ...
    if (!form.category) {
      setAlert("Selecciona una categoría");
      return;
    }
    if (!form.price || Number(form.price) <= 0) {
      setAlert("El precio debe ser mayor a 0");
      return;
    }
    ...
    <div>
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-gray-500 text-sm mt-1">Gestión de stock de productos</p>
        </div>
        <div className="flex items-center gap-2">
          <button
            onClick={() => navigate("/inventory/products/movements")}
